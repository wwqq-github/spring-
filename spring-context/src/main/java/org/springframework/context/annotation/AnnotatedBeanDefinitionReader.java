...
public class AnnotatedBeanDefinitionReader {
	 ...
	 */
	//BeanDefinitionRegistry bean的定义注册 创建的对象作为参数
	...
	public AnnotatedBeanDefinitionReader(BeanDefinitionRegistry registry, Environment environment) {
		//判断registry是否为null 一般不是null
		Assert.notNull(registry, "BeanDefinitionRegistry must not be null");
		//判断environment是否为null 一般不是null
		Assert.notNull(environment, "Environment must not be null");
		//给registry赋值 是new的类型  转型成BeanDefinitionRegistry
	...
	public void registerBean(Class<?> beanClass) {
		//doRegisterBean(test.SpringConfig,null)
	...
	<T> void doRegisterBean(Class<T> beanClass, @Nullable Supplier<T> instanceSupplier, @Nullable String name,
		...
		//setBeanClass(test.SpringConfig);
		//this.metadata = new StandardAnnotationMetadata(test.SpringConfig, true);
		AnnotatedGenericBeanDefinition abd = new AnnotatedGenericBeanDefinition(beanClass);
		//this.conditionEvaluator.shouldSkip(abd.getMetadata())==false
	...
	private static Environment getOrCreateEnvironment(BeanDefinitionRegistry registry) {
		//判断registry 是否为null
		Assert.notNull(registry, "BeanDefinitionRegistry must not be null");
		//不为空 一般执行if  ApplicationContext实现类EnvironmentCapable接口
		if (registry instanceof EnvironmentCapable) {
			//调用AbstractApplicationContext.getEnvironment()方法 这个方法被调用了好几次
